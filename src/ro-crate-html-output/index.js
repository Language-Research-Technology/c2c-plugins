...
function contentChildRefs(entity) {
  const refs = new Set();
  // pcdm:hasMember is how buildCrate() (crate.js) actually links the root
  // dataset to its top-level collections/objects, and how a
  // RepositoryCollection lists its nested objects — hasPart is only what a
  // RepositoryObject uses for its own files. Both must be checked at every
  // level, or a real crate's root (which carries no bare hasPart at all)
  // yields zero children and the whole subtree silently "kept 0 of 0".
...
// because this hook runs last among the output:write writers (see the
...
// absolute-URL guard reconcileFileEntities (src/crate.js) uses to tell a
// real on-disk file apart from a File entity that only records a remote
// reference and has nothing to copy.
...
// fixEncodedSlashes (src/crate.js) above the html this runs on — a
// DOM/cheerio parse-and-reserialize risks subtly reformatting markup that
// isn't ours to rewrite, on output real users will deploy as-is. Matching
// each path as one literal (rather than splitting at '#'/'?' first) means a
// file whose own name contains '#' or '?' still matches correctly instead
// of being mistaken for a fragment/query — see preview_assets.js's
// mapAssetUrl in chaos2crate for the in-app-preview equivalent of this same
// hazard.
